...
int init_OdometricLocalizer(OdometricLocalizer *odo, EncoderTypes type,
		...
		float trackWidth, float countsPerRevolution,
		bool leftFlipped, bool rightFlipped) {

	// Set encoder type being used
	odo->encoderType = type;

	// Set encoders and motors in struct
	odo->LeftEncoder_port = leftEncoder;
	odo->RightEncoder_port = rightEncoder;

	odo->leftFlip = leftFlipped;
	odo->rightFlip = rightFlipped;
...
void step_OdometricLocalizer(OdometricLocalizer *odo) {
	...
	if (abs(leftCounts - odo->previousLeftCounts) < 2
			|| abs(rightCounts - odo->previousRightCounts) < 2)
		return;
	...
	float dDistance = 0.5 * (float) (dLeftCounts + dRightCounts)
			* odo->distancePerCount;
